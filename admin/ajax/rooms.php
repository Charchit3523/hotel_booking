<?php
    ...
    if (isset($_POST['rem_image'])){
        $frm_data = filteration($_POST);
        $values = [$frm_data['image_id'], $frm_data['room_id']]; // Assuming 'room_id' is available in your form
        $pre_q = "SELECT * FROM `room_image` WHERE `sr_no`=? AND `room_id`=?";
        $res = select($pre_q, $values, 'ii'); // Assuming both 'sr_no' and 'room_id' are integers
        $img=mysqli_fetch_assoc($res);
        if ($res) {
            if (deleteImage($img['image'], ROOMS_FOLDER)) {
                $q = "DELETE FROM `room_image` WHERE `sr_no`=? AND `room_id`=?";
                $res = delete($q, $values, 'ii'); // Only one value is needed here
                echo $res;
            } else {
                echo 0;
            }
